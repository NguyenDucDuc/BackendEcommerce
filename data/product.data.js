module.exports = {
  products: [
    {
      id: 1,
      name: 'think and grow rich - 13 nguyên tắc nghĩ giàu, làm giàu',
      sku: new Date().toISOString(),
      price: 104200,
      isActive: true,
      desc: '13 Nguyên Tắc Nghĩ Giàu Làm Giàu là cuốn sách “chỉ dẫn” duy nhất chỉ ra những nguồn lực bạn phải có để thành công. Cuốn sách sẽ giúp bạn trở nên giàu có, làm giàu thêm cho cuộc sống của bạn trên tất cả các phương diện của cuộc sống chứ không chỉ về tài chính và vật chất. Những ý tưởng trong cuốn sách Think and Grow rich – 13 nguyên tắc nghĩ giàu, làm giàu bắt nguồn từ những động lực tốt đẹp: “Thành công cá nhân” và “Quan điểm suy nghĩ tích cực”.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754421/ProductImage/think13__0_yj8szl.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 2,
      name: 'Apple iPhone 14 Pro Max 128GB',
      sku: new Date().toISOString(),
      price: 27890000,
      isActive: true,
      desc: 'Điện thoại được trang bị màn hình Dynamic Island siêu ấn tượng với khả năng tuy biến thành nhiều dạng theo điều khiển của người dùng. Nhờ đó người dùng có thể theo dõi và hiển thị nhanh các thông tin như cuộc gọi, chỉ đường, hẹn giờ,...',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754418/ProductImage/ip14__0_oife0d.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 2,
      categoryId: 2,
      attributeGroupId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 3,
      name: 'atomic habits - thay đổi tí hon, hiệu quả bất ngờ',
      sku: new Date().toISOString(),
      price: 186650,
      isActive: true,
      desc: 'Hẳn là khó đồng ý với điều đó. Nhưng nếu bạn thay đổi thêm một chút? Một chút nữa? Rồi thêm một chút nữa? Đến một lúc nào đó, bạn phải công nhận rằng cuộc sống của mình đã chuyển biến nhờ vào một thay đổi nhỏ',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754417/ProductImage/ato__0_jkd9ng.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 3,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 4,
      name: 'trí tuệ do thái',
      sku: new Date().toISOString(),
      price: 101200,
      isActive: true,
      desc: 'Trí tuệ Do Thái là một cuốn sách tư duyđầy tham vọng trong việc nâng cao khả năng tự học tập, ghi nhớ và phân tích - những điều đã khiến người Do Thái vượt trội lên, chiếm lĩnh những vị trí quan trọng trong ngành truyền thông, ngân hàng và những giải thưởng sáng tạo trên thế giới.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754421/ProductImage/tritue__0_bphssy.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 5,
      name: 'payback time - ngày đòi nợ',
      sku: new Date().toISOString(),
      price: 299000,
      isActive: true,
      desc: 'Trong chứng khoán và thị trường tài chính, dám tham gia đầu tư cũng là một thành công tâm lý ban đầu. Dù vậy, 95% nhà đầu tư Việt Nam thuộc nhóm nhà đầu tư nhỏ lẻ với vốn kiến thức tài chính vô cùng hạn chế. ',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754420/ProductImage/pay__0_rfcagc.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 2,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 6,
      name: 'apple iPhone 12',
      sku: new Date().toISOString(),
      price: 14990000,
      isActive: true,
      desc: 'iPhone 12 đẩy nhanh mọi tác vụ với mạng 5G siêu nhanh.1 A14 Bionic, chip nhanh nhất trên điện thoại thông minh. Hệ thống camera kép mới. Với màn hình Super Retina XDR tuyệt đẹp, nay bạn có thể chiêm ngưỡng từng chi tiết hình ảnh vô cùng sống động.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754417/ProductImage/ip12__0_yiiy01.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 3,
      categoryId: 2,
      attributeGroupId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 7,
      name: 'macBook pro m2',
      sku: new Date().toISOString(),
      price: 39840000,
      isActive: true,
      desc: 'MacBook Pro 14 inch nay đa năng hơn bao giờ hết. Siêu mạnh mẽ với chip M2 thế hệ tiếp theo, đây là chiếc máy tính xách tay chuyên nghiệp nhỏ gọn nhất của Apple, cùng thời lượng pin lên đến 20 giờ',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754419/ProductImage/macpro__0_gbrhub.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 3,
      attributeGroupId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 8,
      name: 'laptop LG gram 2022 17Z90Q-G.AH74A5',
      sku: new Date().toISOString(),
      price: 33990000,
      isActive: true,
      desc: 'Dòng gram 2022 Phiên bản gram 17 có khối lượng rất ấn tượng là 1350g, trọng lượng siêu nhẹ vốn là ưu điểm nổi bật của dòng máy này, với trọng lượng này chỉ cần 1 tay cũng có thể nâng máy thoải mái, dễ dàng để vào túi xách hay balo di chuyển tên mọi hành trình, phù hợp với đối tượng hay phải di chuyển nhiefu như doanh nhân.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754419/ProductImage/lg__0_nhj5y2.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 2,
      categoryId: 3,
      attributeGroupId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 9,
      name: 'Áo AEROREADY Essentials Nam Đen',
      sku: new Date().toISOString(),
      price: 850000,
      isActive: true,
      desc: 'Chiếc áo polo adidas này chính là vũ khí bí mật trong tủ đồ của bạn. Thiết kế polo classic nay được nâng cấp thêm chất vải AEROREADY thấm hút ẩm giúp bạn luôn cảm thấy thoáng mát suốt ngày dài.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754417/ProductImage/adidas__0_ljb4fy.jpg',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 3,
      categoryId: 4,
      attributeGroupId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 10,
      name: 'Xe máy Honda SH Mode 2022 - Đặc biệt phanh ABS',
      sku: new Date().toISOString(),
      price: 850000,
      isActive: true,
      desc: 'Thuộc phân khúc xe ga cao cấp và thừa hưởng thiết kế sang trọng nổi tiếng của dòng xe SH, Sh mode luôn được đánh giá cao nhờ kiểu dáng sang trọng, tinh tế tới từng đường nét, động cơ tiên tiến và các tiện nghi cao cấp xứng tầm phong cách sống thời thượng, đẳng cấp.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754421/ProductImage/sh__0_nyzwmf.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 5,
      attributeGroupId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 11,
      name: 'ví nam cầm tay cao cấp full hộp WilliamPOLO',
      sku: new Date().toISOString(),
      price: 925000,
      isActive: true,
      desc: 'Kiểu dáng hiện đại - sang trọng - nam tính. Ví da nam , ví nam cao cấp full hộp được gia công tỉ mỉ - chất liệu da mềm , làm quà tặng rất đẹp. Da bò được xử lý đúng theo quy trình nên sử dụng càng lâu thì thắt lưng sẽ càng mềm mại, dẻo dai, bền màu và tăng độ bóng mịn',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754421/ProductImage/tui__0_fwq3d7.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 2,
      categoryId: 6,
      attributeGroupId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 12,
      name: 'Đồng Hồ Nam dây thép Casio Edifice',
      sku: new Date().toISOString(),
      price: 4498000,
      isActive: true,
      desc: 'Giới thiệu Dòng ECB-20 kết nối điện thoại thông minh từ EDIFICE, mẫu đồng hồ ghi thời gian hiệu suất cao, kết hợp các phản hồi trực tiếp từ các đội đua để tạo ra những chiếc đồng hồ mới tiên tiến theo khái niệm “Tốc độ & Trí tuệ.”',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754418/ProductImage/dongho__0_lhrlk1.png',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 3,
      categoryId: 19,
      attributeGroupId: 19,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 13,
      name: 'Canon EOS 3000D + Lens EF-S 18 - 55mm III',
      sku: new Date().toISOString(),
      price: 12570000,
      isActive: true,
      desc: 'Máy ảnh Canon EOS 3000D được sản xuất nhằm hướng đến đối tượng người dùng mới tiếp cận DSLR, bên cạnh một mức giá vừa phải cùng nhiều điểm thiết kế thuận tiện để phù hợp với người mới bắt đầu như trang bị tay nắm giúp bạn nắm chắc máy ảnh, ống ngắm quang học giúp bạn bắt được những khoảnh khắc thoáng qua, và một bánh xe chính để điều chỉnh, chuyển đổi nhanh giữa các chức năng giúp người dùng dễ dàng thao tác và cho ra những bức ảnh đẹp mắt.”',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754420/ProductImage/mayanh__0_h5bmkd.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 3,
      categoryId: 16,
      attributeGroupId: 16,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // --------------------

    // -------------1

    {
      id: 14,
      name: 'tâm lý học về tiền',
      sku: new Date().toISOString(),
      price: 110000,
      isActive: true,
      desc: 'Tiền bạc có ở khắp mọi nơi, nó ảnh hưởng đến tất cả chúng ta, và khiến phần lớn chúng ta bối rối. Mọi người nghĩ về nó theo những cách hơi khác nhau một chút. Nó mang lại những bài học có thể được áp dụng tới rất nhiều lĩnh vực trong cuộc sống, như rủi ro, sự tự tin, và hạnh phúc. Rất ít chủ đề cung cấp một lăng kính phóng to đầy quyền lực giúp giải thích vì sao mọi người lại hành xử theo cách họ làm hơn là về tiền bạc. Đó mới là một trong những chương trình hoành tráng nhất trên thế giới. Chúng ta hiếm khi lâm vào hoàn cảnh nợ ngập đầu ư? Biết tiết kiệm để dành cho lúc khốn khó hơn ư? Chuẩn bị sẵn sàng cho việc nghỉ hưu? Có những cái nhìn thiết thực về mối quan hệ giữa tiền và hạnh phúc của chúng ta hơn phải không? Chúng ta đều làm những điều điên rồ với tiền bạc, bởi vì chúng ta đều còn khá mới mẻ với trò chơi này và điều có vẻ điên rồ với bạn lại có khi hợp lý với tôi. Nhưng không ai là điên rồ cả – chúng ta đều đưa ra các quyết định dựa trên những trải nghiệm độc đáo riêng có mang vẻ hợp lý với mình ở bất cứ thời điểm nào. Mục đích của cuốn sách này là sử dụng những câu chuyện ngắn để thuyết phục bạn rằng những kỹ năng mềm còn quan trọng hơn khía cạnh lý thuyết của đồng tiền. Thông qua một tập hợp những thử nghiệm và sai lầm của nhiều năm chúng ta đã học được cách trở thành những nông dân giỏi giang hơn, những thợ sửa ống nước nhiều kỹ năng hơn, và những nhà hóa học tiên tiến hơn. Nhưng liệu việc thử nghiệm và sai lầm có dạy chúng ta trở nên giỏi hơn trong cách quản lý tài chính cá nhân của chính mình không?',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/tamlyhoctien__0_mujjei.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // -------------2

    {
      id: 15,
      name: 'người giàu có nhất thành babylon',
      sku: new Date().toISOString(),
      price: 78400,
      isActive: true,
      desc: 'Thành cổ Babylon được biết đến là nơi khởi minh rực rỡ nhất thời cổ đại, nơi có những thành quách đồ sộ, dân cư đông đúc và các hoạt động giao thương diễn ra nhộn nhịp. Vậy, những người ở Babylon đã làm thế nào để trở thành thương gia, làm thế nào để trở thành người giàu nhất? Điều gì đã khiến họ có thể làm chủ vận mệnh và làm chủ tài chính khi chỉ là những nô lệ thấp kém? Cuốn sách Người giàu có nhất thành Babylon sẽ mang đến cho độc giả những câu chuyện về bí mật của sự giàu có, được kể qua lời của những nô lệ, thương gia, thợ thuyền đến từ 4.000 năm trước. Họ sẽ tiết lộ cho bạn những nguyên tắc quan trọng để sở hữu tiền - vàng và tăng sinh lợi nhuận từ số vốn ban đầu. Nếu bạn đang tìm kiếm sự giàu có, hãy nhớ rằng, tiền không được phép đứng yên, nó phải tham gia vào dòng chảy thị trường, tạo ra lợi ích cho chủ sở hữu. Những phương pháp này sẽ khiến bạn phải bất ngờ vì nó không khó khăn, vô cùng đơn giản nhưng lại dễ dàng bị bỏ qua hoặc vô tình lãng quên đi mất tính căn bản của việc làm giàu.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/babylon__0_o0hsrx.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------3

    {
      id: 16,
      name: 'cây cam ngọt của tôi',
      sku: new Date().toISOString(),
      price: 69000,
      isActive: true,
      desc: 'Hãy làm quen với Zezé, cậu bé tinh nghịch siêu hạng đồng thời cũng đáng yêu bậc nhất, với ước mơ lớn lên trở thành nhà thơ cổ thắt nơ bướm. Chẳng phải ai cũng công nhận khoản “đáng yêu” kia đâu nhé. Bởi vì, ở cái xóm ngoại ô nghèo ấy, nỗi khắc khổ bủa vây đã che mờ mắt người ta trước trái tim thiện lương cùng trí tưởng tượng tuyệt vời của cậu bé con năm tuổi. Có hề gì đâu bao nhiêu là hắt hủi, đánh mắng, vì Zezé đã có một người bạn đặc biệt để trút nỗi lòng: cây cam ngọt nơi vườn sau. Và cả một người bạn nữa, bằng xương bằng thịt, một ngày kia xuất hiện, cho cậu bé nhạy cảm khôn sớm biết thế nào là trìu mến, thế nào là nỗi đau, và mãi mãi thay đổi cuộc đời cậu. Mở đầu bằng những thanh âm trong sáng và kết thúc lắng lại trong những nốt trầm hoài niệm, Cây cam ngọt của tôi khiến ta nhận ra vẻ đẹp thực sự của cuộc sống đến từ những điều giản dị như bông hoa trắng của cái cây sau nhà, và rằng cuộc đời thật khốn khổ nếu thiếu đi lòng yêu thương và niềm trắc ẩn. Cuốn sách kinh điển này bởi thế không ngừng khiến trái tim người đọc khắp thế giới thổn thức, kể từ khi ra mắt lần đầu năm 1968 tại Brazil.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/caycam_jtbvbs.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------4

    {
      id: 17,
      name: 'định luật murphy – mọi bí mật tâm lý thao túng cuộc đời bạn',
      sku: new Date().toISOString(),
      price: 81000,
      isActive: true,
      desc: 'Có phải hằng ngày bạn hay gặp những chuyện dở khóc dở cười tương tự như vậy? Những hiện tượng này đều có thể giải thích bằng một khái niệm tâm lý học thú vị: Định luật Murphy. Nó nhắc nhở chúng ta rằng việc xấu luôn có “cơ may” cao hơn và sai lầm luôn là một phần của thế giới này. Dù cho mỗi chúng ta đều cố gắng hết sức để tránh khỏi sai lầm, nhưng trên thực tế đó là điều bất khả thi. Tuy nhiên, định luật Murphy mang tính cảnh tỉnh và dẫn dắt rất lớn trong cuộc sống hằng ngày. Cuốn sách này giới thiệu đến với bạn đọc về kiến thức cơ bản, hiện tượng thường gặp cùng với các hiệu ứng tâm lý học biểu hiện trong ý thức cá nhân, tính cạnh tranh, quan hệ xã hội,… của định luật Murphy thông qua góc nhìn thực tiễn. Từ đó giúp bạn đọc hiểu được bản chất con người, bản chất xã hội rồi ứng dụng nó vào giải quyết các vấn đề gặp phải trong cuộc sống.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/mulphy__0_f6hwyv.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------5

    {
      id: 18,
      name: 'nhà giả kim',
      sku: new Date().toISOString(),
      price: 52400,
      isActive: true,
      desc: 'Tất cả những trải nghiệm trong chuyến phiêu du theo đuổi vận mệnh của mình đã giúp Santiago thấu hiểu được ý nghĩa sâu xa nhất của hạnh phúc, hòa hợp với vũ trụ và con người. Tiểu thuyết Nhà giả kim của Paulo Coelho như một câu chuyện cổ tích giản dị, nhân ái, giàu chất thơ, thấm đẫm những minh triết huyền bí của phương Đông. Trong lần xuất bản đầu tiên tại Brazil vào năm 1988, sách chỉ bán được 900 bản. Nhưng, với số phận đặc biệt của cuốn sách dành cho toàn nhân loại, vượt ra ngoài biên giới quốc gia, Nhà giả kim đã làm rung động hàng triệu tâm hồn, trở thành một trong những cuốn sách bán chạy nhất mọi thời đại, và có thể làm thay đổi cuộc đời người đọc.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/nhagiakim_0_sdkt49.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------6

    {
      id: 19,
      name: 'càng kỷ luật, càng tự do',
      sku: new Date().toISOString(),
      price: 74000,
      isActive: true,
      desc: 'KỶ LUẬT vốn là ván cờ bạn phải tự đấu với chính mình. Thắng - bạn sẽ có được “bản năng của người mạnh mẽ nhất”, đó là sự tự kiểm soát bản thân. Thua - bạn mãi sống trong cảm giác tạm bợ, nuối tiếc. Càng dễ dàng dung túng cho những thói quen trì hoãn bao nhiêu, cuộc sống của bạn sẽ đi càng nhanh tới sự mất kiểm soát và thiếu quy hoạch bấy nhiêu. Hãy kiên trì đặt ra yêu cầu cao với bản thân, từ chối sự mê hoặc của thói tùy tiện đồng thời thiết lập cho mình những thói quen tốt trong hành vi thường ngày. Sự nỗ lực sẽ luôn được đền đáp xứng đáng nếu bạn biết cách đầu tư công sức và thời gian. Bởi một ngày nào đó trong tương lai, những gì bạn làm sẽ phản chiếu đầy đủ trên chính con người và cuộc đời của bạn. CÀNG KỶ LUẬT, CÀNG TỰ DO - Mong rằng tại thời điểm kết thúc hành trình với cuốn sách, bạn đã là một phiên bản khác kỷ luật hơn, nhưng tự do hơn.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/kyluat__0_wcfst7.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------7

    {
      id: 20,
      name: 'Điềm Tĩnh Và Nóng Giận',
      sku: new Date().toISOString(),
      price: 67200,
      isActive: true,
      desc: 'Trong cuộc sống thường ngày, chúng ta thường nổi giận vì nhiều nguyên do: công việc không suôn sẻ, chúng ta tức giận; bị người khác hiểu nhầm, chúng ta tức giận; thấy việc chướng tai gai mắt, chúng ta tức giận; không thể chấp nhận được dư luận xã hội, chúng ta tức giận. Thậm chí, chúng ta bực tức, cáu gắt, hờn dỗi, nhỏ nhen, uất ức vì thời tiết xấu, vì tiền lương thấp, vì nhà cửa bừa bộn, vì thái độ của người khác, vì những chuyện không may mà mình gặp phải. Dường như cuộc đời chúng ta là một chuỗi tức giận không hồi kết. Hãy thử tự hỏi bản thân: sau khi tức giận thì phiền não sẽ tan biến ư? Dân gian vốn có câu “cả giận mất khôn”, Đức Phật cũng đã dạy “một ngọn lửa sân đốt cháy rừng công đức”. Cơn nóng giận có thể hủy diệt sức khỏe, sự nghiệp, tình cảm và hạnh phúc của chúng ta. Cơn giận khiến đầu óc ta bốc hỏa, kích động và lỗ mãng để rồi phạm phải những sai lầm không thể bù đắp. Nóng giận là trừng phạt bản thân bằng lỗi lầm của người khác, đánh cược hạnh phúc cả đời bằng một phút thiếu kiềm chế. Hầu hết những bất hạnh trong đời đều từ đó mà ra. Bởi thế, mỗi người chúng ta sống trong xã hội này đều cần rèn giũa cho mình một thái độ sống điềm tĩnh trước mọi điều.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/nongvalanh__0_pitp6r.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------8

    {
      id: 21,
      name: 'Rèn Luyện Tư Duy Phản Biện',
      sku: new Date().toISOString(),
      price: 63000,
      isActive: true,
      desc: 'Những người tư duy phản biện cũng biết rằng họ cần thu thập những ý tưởng và đức tin của mọi người. Tư duy phản biện không thể tự nhiên mà có. Những người khác có thể đưa ra những góc nhìn khác mà bạn có thể chưa bao giờ nghĩ tới, và họ có thể chỉ ra những lỗ hổng trong logic của bạn mà bạn đã hoàn toàn bỏ qua. Bạn không cần phải hoàn toàn đồng ý với ý kiến của những người khác, bởi vì điều này cũng có thể dẫn tới những vấn đề liên quan đến thiên kiến, nhưng một cuộc thảo luận phản biện là một bài tập tư duy cực kỳ hiệu quả. Việc lắng nghe những ý kiến của người khác cũng có thể giúp bạn nhận ra rằng phạm vi tri thức của bạn không phải là vô hạn. Không ai có thể biết hết tất cả mọi thứ. Nhưng với việc chia sẻ và đánh giá phê bình kiến thức, chúng ta có thể mở rộng tâm trí. Nếu điều này khiến bạn cảm thấy không thoải mái, không sao cả. Trên thực tế, bước ra ngoài vùng an toàn là một điều quan trọng để mở rộng niềm tin và suy nghĩ của bạn. Tư duy phản biện không phải là chỉ biết vài thứ, và chắc chắn không phải việc xác nhận những điều bạn đã biết. Thay vào đó, nó xoay quanh việc tìm kiếm sự thật – và biến chúng trở thành thứ bạn biết.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/phanbien__0_lutifh.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------9

    {
      id: 22,
      name: '48 Nguyên Tắc Chủ Chốt Của Quyền Lực',
      sku: new Date().toISOString(),
      price: 180000,
      isActive: true,
      desc: 'Quyền lực có sức hấp dẫn vô cùng mạnh mẽ đối với con người trong mọi thời, ở mọi nơi, với mọi giai tầng. Lịch sử xét cho cùng là cuộc đấu tranh triền miên để giành cho bằng được quyền lực cai trị của các tập đoàn thống trị, từ cổ chí kim, từ đông sang tây. Quyền lực là thứ mà rất nhiều người mong muốn nhưng không phải ai cũng dễ dàng đạt được. Vươn lên những vị trí cao hơn trong thang bậc xã hội thường được xem là một khát khao rất con người. Nhưng, liệu có phải chỉ những tài năng xuất chúng mới có thể đạt được điều đó? Không hẳn vậy. Bởi ít ai biết rằng, để giành được một vị trí quyền lực thực tế vẫn mang tính công thức. Qua nghiên cứu lịch sử nhân loại, với những nhân vật có quyền lực nhất tự cổ chí kim, Robert Greene đã khái quát nên 48 nguyên tắc tạo nên quyền lực một cách có cơ sở khoa học. Mỗi nguyên tắc đều được tác giả phân tích, giải thích rõ ràng, mang tính thuyết phục cao và cực kỳ hấp dẫn. Một số quy luật đòi hỏi sự khôn ngoan sắc sảo, một số cần sự lén lút và một số khác hoàn toàn vắng mặt lòng thương xót… Nhưng dù bạn thích hay không thích, tất cả những quy luật này đều có nhiều ứng dụng trong các tình huống thực tế của cuộc đời.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/48quyenluc__0_whjxpc.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------10

    {
      id: 23,
      name: 'Đắc Nhân Tâm',
      sku: new Date().toISOString(),
      price: 67000,
      isActive: true,
      desc: 'Đắc nhân tâm của Dale Carnegie là quyển sách nổi tiếng nhất, bán chạy nhất và có tầm ảnh hưởng nhất của mọi thời đại. Tác phẩm đã được chuyển ngữ sang hầu hết các thứ tiếng trên thế giới và có mặt ở hàng trăm quốc gia. Đây là quyển sách duy nhất về thể loại self-help liên tục đứng đầu danh mục sách bán chạy nhất (best-selling Books) do báo The New York Times bình chọn suốt 10 năm liền. Riêng bản tiếng Anh của sách đã bán được hơn 15 triệu bản trên thế giới. Tác phẩm có sức lan toả vô cùng rộng lớn - dù bạn đi đến bất cứ đâu, bất kỳ quốc gia nào cũng đều có thể nhìn thấy. Tác phẩm được đánh giá là quyển sách đầu tiên và hay nhất, có ảnh hưởng làm thay đổi cuộc đời của hàng triệu người trên thế giới. Không còn nữa khái niệm giới hạn Đắc Nhân Tâm là nghệ thuật thu phục lòng người, là làm cho tất cả mọi người yêu mến mình. Đắc nhân tâm và cái Tài trong mỗi người chúng ta. Đắc Nhân Tâm trong ý nghĩa đó cần được thụ đắc bằng sự hiểu rõ bản thân, thành thật với chính mình, hiểu biết và quan tâm đến những người xung quanh để nhìn ra và khơi gợi những tiềm năng ẩn khuất nơi họ, giúp họ phát triển lên một tầm cao mới. Đây chính là nghệ thuật cao nhất về con người và chính là ý nghĩa sâu sắc nhất đúc kết từ những nguyên tắc vàng của Dale Carnegie.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/dacnhan__0_kq7qxu.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------11

    {
      id: 24,
      name: 'Xây Dựng Một Nhóm Kinh Doanh Thành Công',
      sku: new Date().toISOString(),
      price: 59000,
      isActive: true,
      desc: '“Kinh doanh không phải là khó, dùng người mới là khó”. Trong cuốn sách này bạn sẽ học được cách làm thế nào để xây dựng được một đội nhóm kinh doanh hoạt động với tinh thần chung sức và ngày càng mạnh mẽ hơn trước những thách thức khó khăn hơn. Giá sản phẩm trên Tiki đã bao gồm thuế theo luật hiện hành. Bên cạnh đó, tuỳ vào loại sản phẩm, hình thức và địa chỉ giao hàng mà có thể phát sinh thêm chi phí khác như phí vận chuyển, phụ phí hàng cồng kềnh, thuế nhập khẩu (đối với đơn hàng giao từ nước ngoài có giá trị trên 1 triệu đồng).....',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/kinhdoanh__0_zpszdu.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------12

    {
      id: 25,
      name: '7 Thói quen hiệu quả - The 7 Habits Of Highly Effective People',
      sku: new Date().toISOString(),
      price: 171900,
      isActive: true,
      desc: 'Mơ ước của hầu hết mọi người trong đời là trở thành một con người thành công và hạnh phúc bằng chính tài năng và đạo đức của mình. Ở bình diện tổ chức hay các quốc gia cũng vậy. Một tổ chức hay quốc gia càng có nhiều người hướng đến thành công bằng tài năng và đạo đức, tổ chức hay quốc gia đó càng có cơ hội phát triển thịnh vượng và văn minh. Tài năng và đạo đức đã trở thành những phẩm chất được tìm kiếm và tôn vinh hàng đầu trong mọi xã hội và mọi thời đại, và chúng có một mối liên quan chặt chẽ với hiệu quả và hiệu quả bền vững. Cần có tài năng thì mới tạo ra được hiệu quả, cần có đạo đức thì mới tạo ra được hiệu quả bền vững. Và ngược lại, không ai có thể được xem là tài năng nếu không tạo ra hiệu quả, cũng như không ai có thể được coi là có đạo đức nếu như việc tạo ra hiệu quả đó chỉ là nhất thời chứ không mang tính bền vững, bởi lẽ, để tạo ra hiệu quả bền vững thì luôn phải dựa trên nền tảng đạo đức. Vì thế, câu trả lời về “hiệu quả” cũng chính là “lời đáp” cho câu chuyện về tài năng và đạo đức mà nhiều người tìm kiếm, mà lại là một “lời đáp” được đúc kết bằng việc chuyển hóa một hệ thống giá trị ở bên trong để hình thành bản tính thành công bền vững của con người, chứ hoàn toàn không phải là những chiêu trò hay thủ thuật ở bên ngoài. Stephen Covey từng nói rằng, ông không phát minh hay tạo ra mà được đưa ra dựa trên những giá trị phổ quát và những nguyên tắc trường tồn của nhân loại. Chính trong thái độ khiêm nhường đó, ta nhận ra tầm vóc của ông. Vì ông đã tạo ra một cầu nối giữa chúng ta với những tri thức tinh hoa của nhân loại về làm người, về giá trị sống, và nhất là về tính hiệu quả. Thường những tri thức này không phải là thứ dễ tiếp cận, dễ tiếp thu với đại chúng, nhưng bằng việc sắp xếp và tổng hợp chúng trong một hình hài có tính hệ thống và tính ứng dụng cao, ông đã góp phần đưa những tinh hoa tri thức đó đến với công chúng dễ dàng hơn. Và cũng bởi được xây dựng dựa trên những gì phổ quát và trường tồn nên “7 Habits” mới trở thành một nền văn hóa vượt không gian, vượt thời gian, dành cho tất cả mọi người, mọi tổ chức và mọi xứ sở. Bất kỳ ai cài đặt nền văn hóa “7 habits” này vào và sống hàng ngày với nền văn hóa đó, thì dù là một tổng thống, một doanh nhân, một quản lý hay một đầu bếp, một nhân viên cũng sẽ trở nên hiệu quả hơn hẳn.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/7hatbit__0_gdwhnu.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------13

    {
      id: 26,
      name: 'Bắt Đầu Với Câu Hỏi Tại Sao - Nghệ Thuật Truyền Cảm Hứng Trong Kinh Doanh',
      sku: new Date().toISOString(),
      price: 82000,
      isActive: true,
      desc: 'Lần đầu tiên khi tôi khám phá ra sức mạnh của lý do TẠI SAO là khi tôi cần đến nó trong cuộc đời mình. Nó không phải là kết quả của việc theo đuổi con đường học vấn của tôi. Nó đã xuất hiện khi tôi không còn cảm hứng với công việc của mình và cảm thấy vô cùng chán nản và thất vọng. Thực tế là đã không có vấn đề gì xảy ra với công việc của tôi, mà do tôi đã đánh mất niềm vui trong công việc mình làm. Nếu xét trên phương diện bề ngoài thì tôi nên vui mới phải. Tôi đã nhận được những khoản thù lao tốt và được làm việc với những đối tác tuyệt vời. Nhưng tôi đã trở nên vô cảm với những gì mình làm. Tôi không còn cảm thấy hài lòng với công việc của mình và cần tìm ra cách để nhóm lên ngọn lửa nhiệt huyết và đam mê thêm một lần nữa. Việc khám phá tầm quan trọng của lý do TẠI SAO đã hoàn toàn thay đổi cái nhìn của tôi về thế giới. Và việc khám phá ra lý do TẠI SAO của chính mình đã giúp tôi tìm lại được niềm đam mê còn cháy bỏng hơn gấp bội so với trước đây. Tôi đã chia sẻ ý tưởng vô cùng đơn giản, mạnh mẽ và thực tế này với bạn bè. Khi học hỏi được điều gì có giá trị, chúng tôi thường chia sẻ nó với những người mình yêu mến. Thật tuyệt vời vì những người bạn tôi đã bắt đầu có những thay đổi lớn lao trong cuộc đời. Và họ đã mời tôi chia sẻ ý tưởng này với bạn bè của họ và những người mà họ yêu mến. Từ đó ý tưởng này bắt đầu được lan truyền.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/batdaucauhoitaisao__0_jjfzfn.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------14

    {
      id: 27,
      name: 'Luật Hấp Dẫn',
      sku: new Date().toISOString(),
      price: 83500,
      isActive: true,
      desc: 'Sức mạnh của suy nghĩ, sức mạnh của linh hồn bên trong vượt quá những gì mà tâm trí logic có thể lý giải. Chúng ta không phải là những con người đang đi trên cuộc hành trình (tìm kiếm và khai sáng) tâm linh. Chúng ta là những linh hồn đang đi trên cuộc hành trình của con người. Vậy bạn muốn cuộc hành trình này sẽ thế nào? Trong cuốn sách này, bạn sẽ bắt đầu với việc khám phá về sức mạnh của suy nghĩ với sự tồn tại của việc thao túng hay chuyên chế vô hình giữa tâm trí này lên tâm trí khác, lựa chọn cách bạn tiếp nhận và kiểm soát dòng năng lượng đến từ xung quanh. Ý nghĩ là vật thể. Ý nghĩ là sức mạnh. Sự khát vọng và khao khát mang lực hấp dẫn. Sự thành thật là nguồn sức mạnh bên trong của con người. Chúng ta sẽ học cách ý thức về sức mạnh nội tại này và chuyển hướng dòng suy nghĩ của mình đến những điều tích cực. Chào mừng bạn đến với thế giới ý thức, để nắm lấy sức mạnh chữa lành từ bên trong, làm chủ nguồn năng lượng vũ trụ và “hấp dẫn” thành công, sức khỏe và hạnh phúc.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/luathapdan_o2zgkp.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------15

    {
      id: 28,
      name: 'Ba người thầy vĩ đại',
      sku: new Date().toISOString(),
      price: 74300,
      isActive: true,
      desc: 'Cho dù tôi có thu thập được bao nhiêu tài sản vật chất đi chăng nữa thì cái thằng người mà tôi nhìn thấy trong tấm gương phòng tắm mỗi buổi sáng vẫn y nguyên – tôi không hề hạnh phúc hơn và không hề cảm thấy tốt hơn tí nào. Suy ngẫm nhiều hơn về thực trạng cuộc sống của mình, tôi bắt đầu nhận thức được sự trống rỗng ngay trong tim mình. Tôi bắt đầu chú ý đến những tiếng thầm thì lặng lẽ của con tim, những điều chỉ dẫn tôi rời bỏ nghề nghiệp mình đã chọn và bắt đầu quá trình tìm kiếm tâm hồn một cách nghiêm túc. Tôi bắt đầu suy nghĩ về lý do tại sao tôi lại ở đây, trên hành tinh này, và nhiệm vụ cụ thể của tôi là gì. Tôi tự hỏi tại sao cuộc đời mình lại không có tác dụng và cần phải thực hiện những thay đổi sâu sắc nào để giúp mình đi đúng hướng. Tôi xem xét những niềm tin cốt lõi, những giả định, và những lăng kính mà mình nhìn ra thế giới, và tôi tự cam kết làm sạch những lăng kính không lành mạnh.” “Cuốn sách này là một tác phẩm hư cấu. Đây là câu chuyện về một người đàn ông có tên Jack Valentine mà đường đời có nhiều điểm giống với tôi. Có cảm nhận rất không đầy đủ với tư cách một con người, anh ấy lên kế hoạch tìm kiếm tri thức để sống một cuộc sống hạnh phúc hơn, khoẻ khoắn hơn và đẹp hơn.”',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760827/ProductImage/3nguoithay_mltn2b.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------16

    {
      id: 29,
      name: 'Sách Sức Mạnh Tiềm Thức',
      sku: new Date().toISOString(),
      price: 94500,
      isActive: true,
      desc: 'Là một trong những quyển sách về nghệ thuật sống nhận được nhiều lời ngợi khen và bán chạy nhất mọi thời đại, Sức Mạnh Tiềm Thức đã giúp hàng triệu độc giả trên thế giới đạt được những mục tiêu quan trọng trong đời chỉ bằng một cách đơn giản là thay đổi tư duy. Sức Mạnh Tiềm Thức giới thiệu và giải thích các phương pháp tập trung tâm thức nhằm xoá bỏ những rào cản tiềm thức đã ngăn chúng ta đạt được những điều mình mong muốn. Sức Mạnh Tiềm Thức không những có khả năng truyền cảm hứng cho người đọc, mà nó còn rất thực tế vì được minh hoạ bằng những ví dụ sinh động trong cuộc sống hằng ngày. Từ đó, Sức Mạnh Tiềm Thức giúp độc giả vận dụng năng lực trí tuệ phi thường tiềm ẩn troing mỗi người để tạo dựng sự tự tin, xây dựng các mối quan hệ hoà hợp, đạt được thành công trong sự nghiệp, vượt qua những nỗi sợ hãi và ám ảnh, xua đi những thói quen tiêu cực, và thậm chí còn hướng dẫn cách ta chữa lành những vết thương về thể chất cũng như tâm hồn để có sự bình an, hạnh phúc trọn vẹn trong cuộc sống.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/sucmanhteimthuc__0_tgwckq.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------17

    {
      id: 30,
      name: 'Ikigai - Bí Mật Sống Trường Thọ Và Hạnh Phúc Của Người Nhật',
      sku: new Date().toISOString(),
      price: 104250,
      isActive: true,
      desc: 'IKIGAI là tập hợp nhiều câu chuyện đời, chuyện nghề giản dị, lý giải tường minh năm điều cốt lõi của ikigai –triết lý sống mang lại hạnh phúc và trường thọ của người Nhật: Bắt đầu từ những việc nhỏ nhặt Giải phóng cái tôi cá nhân Sự hài hòa và tính bền vững Niềm vui từ những điều nhỏ bé Sống trọn khoảnh khắc hiện tại. Nhưng làm thế nào để bạn tìm được ikigai của mình? Có nhất thiết phải có sự nghiệp thành công như Jiro Ono – ông chủ nhà hàng Sukiyabashi Jiro, nơi vinh dự được tiếp đón Obama, hay phải trở thành một yokozuna trong môn vật sumo mới đạt tới ikigai? Thông qua những nghiên cứu khoa học và từ chính trải nghiệm của bản thân, Ken Mogi mang tới cho độc giả một nhận thức sâu sắc về ikigai, đồng thời hiểu thêm về lịch sử và văn hóa Nhật Bản. Những giá trị của ikigai mà bạn thu nhận từ sách có thểgiúp bạn tìm ra ikigai của mình, và quan trọng hơn hết, mỗi sớm mai thức dậy biết mình đang sống vì điều gì.',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686761461/ProductImage/ikigai__0_hbh6dx.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------18

    {
      id: 31,
      name: 'Từ Tốt Đến Vĩ Đại',
      sku: new Date().toISOString(),
      price: 91000,
      isActive: true,
      desc: 'Jim Collins cùng nhóm nghiên cứu đã miệt mài nghiên cứu những công ty có bước nhảy vọt và bền vững để rút ra những kết luận sát sườn, những yếu tố cần kíp để đưa công ty từ tốt đến vĩ đại. Đó là những yếu tố khả năng lãnh đạo, con người, văn hóa, kỷ luật, công nghệ… Những yếu tố này được nhóm nghiên cứu xem xét tỉ mỉ và kiểm chứng cụ thể qua 11 công ty nhảy vọt lên vĩ đại. Mỗi kết luận của nhóm nghiên cứu đều hữu ích, vượt thời gian, ý nghĩa vô cùng to lớn đối với mọi lãnh đạo và quản lý ở mọi loại hình công ty (từ công ty có nền tảng và xuất phát tốt đến những công ty mới khởi nghiệp), và mọi lĩnh vực ngành nghề. Đây là cuốn sách nên đọc đối với bất kỳ lãnh đạo hay quản lý nào!',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844327/tutottoividai__0_tvqg5k.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------19

    {
      id: 32,
      name: 'Chiến Tranh Tiền Tệ',
      sku: new Date().toISOString(),
      price: 109600,
      isActive: true,
      desc: 'Thứ nhất, kinh tế Âu Mỹ sẽ rơi vào tình cảnh tiêu điều trường kỳ (chí ít là 10 năm), cho dù có nới lỏng chính sách về tiền tệ, hay kích thích chính sách tài chính – về cơ bản đều vô hiệu; Thứ hai, khi đó “lượng khí thải carbon” vẫn là một khái niệm tương đối xa lạ với xã hội Trung Quốc, sẽ phát huy tác dụng ngày càng quan trọng đối với kinh tế và xã hội, và sẽ bị “tài chính hóa” và “tiền tệ hóa”; Thứ 3, loại tiền tệ chủ quyền sẽ từng bước bị loại tiền tệ khu vực thay thế, và cuốn cùng sẽ tiến hóa hướng đến sự đơn nhất về tiền tệ trên thế giới. Và đến nay, ba dự đoán đó đều đã trở thành sự thực. Mục đích của cuốn sách này không phải là để dạy bạn cách đầu tư, cách phân bổ tài sản hay dạy một bộ phương pháp đối phó tiền tệ điển hình. Cuốn sách này nhằm trả lời những câu hỏi từ lâu đã khiến chúng ta bối rối và chưa được giải đáp: Tại sao tiền tệ lại có xung đột?',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844326/chientrangtinte__0_cmljbg.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------20

    {
      id: 33,
      name: 'Kỹ Năng Bán Hàng Tuyệt Đỉnh',
      sku: new Date().toISOString(),
      price: 78000,
      isActive: true,
      desc: 'Kỹ năng bán hàng tuyệt đỉnh sẽ cho bạn đọc biết được những quy tắc bán hàng, vốn là điều kiện tiên quyết để gặt hái thành công trong bất kỳ lĩnh vực nào, với bất kỳ ai, bất kỳ ở đâu. Trong từng trang sách, tác giả Cardone giới thiệu và phân tích kỹ lưỡng những kỹ thuật và bước tiếp cận thiết yếu để bạn có thể làm chủ nghệ thuật bán hàng trong mọi tình huống. Bạn sẽ học được cách xử trí khi bị khách hàng từ chối, cách lật ngược tình thế, rút ngắn quy trình bán hàng và làm sao để bản thân ngày càng chuyên nghiệp hơn. Cardone cũng sẽ chỉ cho bạn thấy đâu là những yếu tố quyết định thành công, sẽ luôn giúp bạn: * Bán được hàng ngày cả trong điều kiện kinh tế khó khăn * Vượt qua nỗi e sợ không được khách hàng phản hồi * Làm đầy phễu bán hàng với các thương vụ mới * Luôn có thái độ tích cực, ngay cả khi đã bị khước từ',
      image:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844327/kynangbanhang__0_pgrtz0.webp',
      unitInStock: 100,
      unitOnOrder: 0,
      rate: 0,
      shopId: 1,
      categoryId: 12,
      attributeGroupId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
  ],
  productVarchar: [
    {
      id: 1,
      value: 'Napoleon Hill',
      productId: 1,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 2,
      value: 'Thái Hà',
      productId: 1,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 3,
      value: '15.5 x 24 cm',
      productId: 1,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 4,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 1,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 5,
      value: 'Bìa mềm',
      productId: 1,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 6,
      value: 'Nhà Xuất Bản Lao Động',
      productId: 1,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 7,
      value: 'Apple',
      productId: 2,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 8,
      value: 'Mỹ',
      productId: 2,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 9,
      value: '6.1 inch',
      productId: 2,
      attributeId: 11,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 10,
      value: 'Chính 48 MP & Phụ 12 MP, 12 MP',
      productId: 2,
      attributeId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 11,
      value: '12 MP',
      productId: 2,
      attributeId: 13,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 12,
      value: 'Tím',
      productId: 2,
      attributeId: 14,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 13,
      value: '128GB',
      productId: 2,
      attributeId: 15,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 14,
      value: '175g',
      productId: 2,
      attributeId: 10,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 15,
      value: 'James Clear',
      productId: 3,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 16,
      value: 'Phương Nam Book',
      productId: 3,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 17,
      value: '14 x 20.5 cm',
      productId: 3,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 18,
      value: 'Bìa mềm',
      productId: 3,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 19,
      value: 'Nhà Xuất Bản Thế Giới',
      productId: 3,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 20,
      value: 'Eran Katz',
      productId: 4,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 21,
      value: 'Alphabooks',
      productId: 4,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 22,
      value: '13 x 20,5 cm',
      productId: 4,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 23,
      value: 'Phương Oanh',
      productId: 4,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 24,
      value: 'Bìa mềm',
      productId: 4,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 25,
      value: 'Nhà Xuất Bản Lao Động Xã Hội',
      productId: 4,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 26,
      value: 'Phil Town',
      productId: 5,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 27,
      value: 'CÔNG TY TNHH HAPPY LIVE',
      productId: 5,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 28,
      value: '16 x 24 cm',
      productId: 5,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 29,
      value: 'Nguyễn Hoàng Nam, Thái Phạm',
      productId: 5,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 30,
      value: 'Bìa cứng',
      productId: 5,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 31,
      value: 'Nhà Xuất Bản Thế Giới',
      productId: 5,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 32,
      value: 'Apple',
      productId: 6,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 33,
      value: 'Mỹ',
      productId: 6,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 34,
      value: '6.1 inch',
      productId: 6,
      attributeId: 11,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 35,
      value: '12 MP + 12 MP',
      productId: 6,
      attributeId: 12,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 36,
      value: '12 MP',
      productId: 6,
      attributeId: 13,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 37,
      value: 'Đen',
      productId: 6,
      attributeId: 14,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 38,
      value: '128GB',
      productId: 6,
      attributeId: 15,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 39,
      value: '164g',
      productId: 6,
      attributeId: 10,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 40,
      value: 'Apple',
      productId: 7,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 41,
      value: 'Apple M2 Pro',
      productId: 7,
      attributeId: 16,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 42,
      value: '16-core GPU',
      productId: 7,
      attributeId: 17,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 43,
      value: '512GB',
      productId: 7,
      attributeId: 18,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 44,
      value: '32GB',
      productId: 7,
      attributeId: 19,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 45,
      value: '14 inch',
      productId: 7,
      attributeId: 11,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 46,
      value: 'Mỹ',
      productId: 7,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 47,
      value: '1.6kg',
      productId: 7,
      attributeId: 10,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 48,
      value: 'LG',
      productId: 8,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 49,
      value: 'Intel Core i7-1260P',
      productId: 8,
      attributeId: 16,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 50,
      value: 'Intel Iris Xe',
      productId: 8,
      attributeId: 17,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 51,
      value: '512GB',
      productId: 8,
      attributeId: 18,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 52,
      value: '32GB',
      productId: 8,
      attributeId: 19,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 53,
      value: '15.6 inch',
      productId: 8,
      attributeId: 11,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 54,
      value: 'williampolo',
      productId: 11,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 55,
      value: 'Trung Quốc',
      productId: 11,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 56,
      value: '27*16.5*3.3 cm',
      productId: 11,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 57,
      value: 'Casio',
      productId: 12,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 58,
      value: 'Nhật Bản',
      productId: 12,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 59,
      value: 'ECB-20DB-1ADF',
      productId: 12,
      attributeId: 22,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 60,
      value: 'Canon',
      productId: 13,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 61,
      value: 'Nhật Bản',
      productId: 13,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 62,
      value: 'EOS3000D',
      productId: 13,
      attributeId: 22,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 63,
      value: 'APS-C CMOS',
      productId: 13,
      attributeId: 23,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 64,
      value: 'DIGIC 4+',
      productId: 13,
      attributeId: 24,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 65,
      value: 'EF-S 18-55mm f/3.5-5.6 IS III',
      productId: 13,
      attributeId: 25,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 66,
      value: '3 inch',
      productId: 13,
      attributeId: 11,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 67,
      value: '129.0 x 101.6 x 77.1 mm',
      productId: 13,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 68,
      value: '436g',
      productId: 13,
      attributeId: 10,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    {
      id: 69,
      value: 'Hàn Quốc',
      productId: 9,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 70,
      value: '1350 g',
      productId: 9,
      attributeId: 10,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 71,
      value: 'Adidas',
      productId: 9,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 72,
      value: 'Khác',
      productId: 9,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 73,
      value: 'chất vải AEROREADY',
      productId: 9,
      attributeId: 20,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 74,
      value: 'Form Slimfit ôm vừa người, tôn dáng',
      productId: 9,
      attributeId: 21,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 75,
      value: 'Honda',
      productId: 10,
      attributeId: 8,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 76,
      value: 'Nhật Bản',
      productId: 10,
      attributeId: 9,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // ------------------------------
    // -----------------------1

    {
      id: 77,
      value: 'Morgan Housel',
      productId: 14,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 78,
      value: '1980 Books',
      productId: 14,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 79,
      value: '13 x 20.5 cm',
      productId: 14,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 80,
      value: 'Hoàng Thị Minh Phúc',
      productId: 14,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 81,
      value: 'Bìa mềm',
      productId: 1,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 82,
      value: 'Nhà Xuất Bản Dân Trí',
      productId: 14,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // ------------------2

    {
      id: 83,
      value: 'GEORGE SAMUEL CLASON',
      productId: 15,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 84,
      value: 'SBOOKS',
      productId: 15,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 85,
      value: '15.5 x 24 cm',
      productId: 15,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 86,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 15,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 87,
      value: 'Bìa mềm',
      productId: 15,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 88,
      value: 'Nhà Xuất Bản Văn Học',
      productId: 15,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------3

    {
      id: 89,
      value: 'José Mauro de Vasconcelos',
      productId: 16,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 90,
      value: '	Nhã Nam',
      productId: 16,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 91,
      value: '15.5 x 24 cm',
      productId: 16,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 92,
      value: 'Nguyễn Bích Lan , Tô Yến Ly',
      productId: 16,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 93,
      value: 'Bìa mềm',
      productId: 16,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 94,
      value: 'Nhà Xuất Bản Hội Nhà Văn',
      productId: 16,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------------------4

    {
      id: 95,
      value: 'Napoleon Hill',
      productId: 17,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 96,
      value: 'Thái Hà',
      productId: 17,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 97,
      value: '15.5 x 24 cm',
      productId: 17,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 98,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 17,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 99,
      value: 'Bìa mềm',
      productId: 17,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 100,
      value: 'Nhà Xuất Bản Lao Động',
      productId: 17,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------------/5

    {
      id: 101,
      value: 'Paulo Coelho',
      productId: 18,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 102,
      value: 'Nhã Nam',
      productId: 18,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 103,
      value: '15.5 x 24 cm',
      productId: 18,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 104,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 18,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 105,
      value: 'Bìa mềm',
      productId: 18,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 106,
      value: 'Nhà Xuất Bản Hà Nội',
      productId: 18,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------------------------6

    {
      id: 107,
      value: 'Ca Tây',
      productId: 19,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 108,
      value: 'Bloom Books',
      productId: 19,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 109,
      value: '15.5 x 24 cm',
      productId: 19,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 110,
      value: 'Tuyết Mai',
      productId: 19,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 111,
      value: 'Bìa mềm',
      productId: 19,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 112,
      value: 'Nhà Xuất Bản Thế Giới',
      productId: 19,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------7

    {
      id: 113,
      value: 'Tạ Quốc Kế',
      productId: 20,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 114,
      value: 'Bookland',
      productId: 20,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 115,
      value: '15.5 x 24 cm',
      productId: 20,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 116,
      value: 'Giang Lê',
      productId: 20,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 117,
      value: 'Bìa mềm',
      productId: 20,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 118,
      value: 'Nhà Xuất Bản Thế Giới',
      productId: 20,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // --------------------------------8

    {
      id: 119,
      value: 'Albert Rutherford',
      productId: 21,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 120,
      value: '1980 Books',
      productId: 21,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 121,
      value: '15.5 x 24 cm',
      productId: 21,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 122,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 21,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 123,
      value: 'Bìa mềm',
      productId: 21,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 124,
      value: 'Nhà Xuất Bản Phụ Nữ',
      productId: 21,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------------9

    {
      id: 125,
      value: 'Robert Greene',
      productId: 22,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 126,
      value: 'NXB Trẻ',
      productId: 22,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 127,
      value: '15.5 x 24 cm',
      productId: 22,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 128,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 22,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 129,
      value: 'Bìa mềm',
      productId: 22,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 130,
      value: 'NXB Trẻ',
      productId: 22,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ------------------------------10

    {
      id: 131,
      value: 'Dale Carnegie',
      productId: 23,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 132,
      value: 'First News - Trí Việt',
      productId: 23,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 133,
      value: '15.5 x 24 cm',
      productId: 23,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 134,
      value: 'Nguyễn Văn Phước',
      productId: 23,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 135,
      value: 'Bìa mềm',
      productId: 23,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 136,
      value: 'Nhà Xuất Bản Tổng hợp TP.HCM',
      productId: 23,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------------------11

    {
      id: 137,
      value: 'Blair Singer',
      productId: 24,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 138,
      value: 'NXB Trẻ',
      productId: 24,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 139,
      value: '15.5 x 24 cm',
      productId: 24,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 140,
      value: 'Ross Cornwell -Thảo Triều',
      productId: 24,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 141,
      value: 'Bìa mềm',
      productId: 24,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 142,
      value: 'NXB Trẻ',
      productId: 24,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // --------------------12

    {
      id: 143,
      value: 'Stephen R. Covey',
      productId: 25,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 144,
      value: 'Viện Quản lý PACE',
      productId: 25,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 145,
      value: '15.5 x 24 cm',
      productId: 25,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 146,
      value: 'Hoàng Trung, Trí Thể, Uyên Phương, Thanh Hà, Trần Huỳnh',
      productId: 25,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 147,
      value: 'Bìa cứng',
      productId: 25,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 148,
      value: 'Nhà Xuất Bản Tổng hợp TP.HCM',
      productId: 25,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------13

    {
      id: 149,
      value: 'Simon Sinek',
      productId: 26,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 150,
      value: 'Thái Hà',
      productId: 26,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 151,
      value: '15.5 x 24 cm',
      productId: 26,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 152,
      value: 'Hoàng Việt',
      productId: 26,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 153,
      value: 'Bìa mềm',
      productId: 26,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 154,
      value: 'Nhà Xuất Bản Công Thương',
      productId: 26,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------14

    {
      id: 155,
      value: 'Prentice Mulford',
      productId: 27,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 156,
      value: '1980 Books',
      productId: 27,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 157,
      value: '13 x 19 cm',
      productId: 27,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 158,
      value: '1980Books dịch',
      productId: 27,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 159,
      value: 'Bìa mềm',
      productId: 27,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 160,
      value: 'Nhà Xuất Bản Thanh Niên',
      productId: 27,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -------------------------15

    {
      id: 161,
      value: 'Robin Sharma',
      productId: 28,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 162,
      value: 'Thái Hà',
      productId: 28,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 163,
      value: '14.5 x 20.5 cm',
      productId: 28,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 164,
      value: 'Nguyễn Xuân Hồng',
      productId: 28,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 165,
      value: 'Bìa mềm',
      productId: 28,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 166,
      value: 'Nhà Xuất Bản Lao Động',
      productId: 28,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // --------------------------16

    {
      id: 167,
      value: 'Joseph Murphyc',
      productId: 29,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 168,
      value: 'First News - Trí Việt',
      productId: 29,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 169,
      value: '15.5 x 24 cm',
      productId: 29,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 170,
      value: 'Bùi Thanh Châu-Mai Sơn',
      productId: 29,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 171,
      value: 'Bìa mềm',
      productId: 29,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 172,
      value: 'Nhà Xuất Bản Tổng hợp TP.HCM',
      productId: 29,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // -----------------------17

    {
      id: 173,
      value: 'Ken Mogi',
      productId: 30,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 174,
      value: 'Nhã Nam',
      productId: 30,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 175,
      value: '15.5 x 24 cm',
      productId: 30,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 176,
      value: 'Vũ Thị Nhân Hậu',
      productId: 30,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 177,
      value: 'Bìa mềm',
      productId: 30,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 178,
      value: 'Nhà Xuất Bản Lao Động',
      productId: 30,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ---------------------18

    {
      id: 179,
      value: 'Jim Collins',
      productId: 31,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 180,
      value: 'NXB Trẻ',
      productId: 31,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 181,
      value: '15.5 x 24 cm',
      productId: 31,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 182,
      value: 'Trần Thị Ngân Tuyến',
      productId: 31,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 183,
      value: 'Bìa mềm',
      productId: 31,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 184,
      value: 'NXB Trẻ',
      productId: 31,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // --------------------19

    {
      id: 185,
      value: 'Song Hong Bing',
      productId: 32,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 186,
      value: 'Bách Việt',
      productId: 32,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 187,
      value: '16x24 cm',
      productId: 32,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 188,
      value: 'Đặng Hồng Quân',
      productId: 32,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 189,
      value: 'Bìa mềm',
      productId: 32,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 190,
      value: 'Nhà Xuất Bản Lao Động',
      productId: 32,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // --------------------20

    {
      id: 191,
      value: 'Grant Cardone',
      productId: 33,
      attributeId: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 192,
      value: 'Saigon Books',
      productId: 33,
      attributeId: 2,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 193,
      value: '14 x 21.5 cm',
      productId: 33,
      attributeId: 3,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 194,
      value: 'Bùi Thị Ngọc Hương',
      productId: 33,
      attributeId: 4,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 195,
      value: 'Bìa mềm',
      productId: 33,
      attributeId: 5,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 196,
      value: 'Nhà Xuất Bản Thế Giới',
      productId: 33,
      attributeId: 7,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
  ],
  productInt: [
    {
      id: 1,
      value: 399,
      productId: 1,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // ---------------------------------------
    // -----------1
    {
      id: 5,
      value: 384,
      productId: 14,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // ------------2

    {
      id: 6,
      value: 208,
      productId: 15,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ------------3
    {
      id: 7,
      value: 244,
      productId: 16,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // ------------4
    {
      id: 8,
      value: 399,
      productId: 17,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // ------------5
    {
      id: 9,
      value: 228,
      productId: 18,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------6
    {
      id: 10,
      value: 288,
      productId: 19,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------7
    {
      id: 11,
      value: 224,
      productId: 20,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------8
    {
      id: 12,
      value: 204,
      productId: 21,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------9

    {
      id: 13,
      value: 400,
      productId: 22,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------10
    {
      id: 14,
      value: 399,
      productId: 23,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------11
    {
      id: 15,
      value: 200,
      productId: 24,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------12
    {
      id: 16,
      value: 536,
      productId: 25,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------13
    {
      id: 17,
      value: 346,
      productId: 26,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------14
    {
      id: 18,
      value: 264,
      productId: 27,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------15
    {
      id: 19,
      value: 330,
      productId: 28,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------16
    {
      id: 20,
      value: 336,
      productId: 29,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------17
    {
      id: 21,
      value: 199,
      productId: 30,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------18
    {
      id: 22,
      value: 484,
      productId: 31,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------19
    {
      id: 23,
      value: 632,
      productId: 32,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    // // -----------20
    {
      id: 24,
      value: 293,
      productId: 33,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // --------------------------
    {
      id: 2,
      value: 385,
      productId: 3,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 3,
      value: 444,
      productId: 4,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 4,
      value: 280,
      productId: 5,
      attributeId: 6,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
  ],
  productImages: [
    {
      id: 1,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754421/ProductImage/think13__0_yj8szl.jpg',
      productId: 1,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 2,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754421/ProductImage/think13__1_bhr7yb.jpg',
      productId: 1,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // -----------------------------

    // ----------------------1

    {
      id: 23,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/tamlyhoctien__0_mujjei.webp',
      productId: 14,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 24,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/tamlyhoctien__1_uhbapz.webp',
      productId: 14,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // ----------------------2

    {
      id: 25,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/babylon__0_o0hsrx.webp',
      productId: 15,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 26,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/babylon__1_t92za6.webp',
      productId: 15,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------3

    {
      id: 27,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/caycam_jtbvbs.webp',
      productId: 16,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 28,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/caycam_jtbvbs.webp',
      productId: 16,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------4

    {
      id: 29,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/mulphy__0_f6hwyv.webp',
      productId: 17,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 30,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/mulphy__1_kzioiy.webp',
      productId: 17,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------5

    {
      id: 31,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/nhagiakim_0_sdkt49.webp',
      productId: 18,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 32,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/luathapdan__1_njale3.webp',
      productId: 18,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------6

    {
      id: 33,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/kyluat__0_wcfst7.webp',
      productId: 19,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 34,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/kyluat__1_j5whoj.webp',
      productId: 19,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------7

    {
      id: 35,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/nongvalanh__0_pitp6r.webp',
      productId: 20,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 36,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/nongvalanh__1_oienw1.webp',
      productId: 20,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------8

    {
      id: 37,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/phanbien__0_lutifh.webp',
      productId: 21,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 38,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/phanbien__1_fvkcdd.webp',
      productId: 21,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------9

    {
      id: 39,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/48quyenluc__0_whjxpc.webp',
      productId: 22,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 40,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/48quyenluc__1_ad37oi.webp',
      productId: 22,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------10

    {
      id: 41,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/dacnhan__0_kq7qxu.webp',
      productId: 23,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 42,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/dacnhan__1_xbvabd.webp',
      productId: 23,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------11

    {
      id: 43,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/kinhdoanh__0_zpszdu.webp',
      productId: 24,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 44,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/kinhdoanh__0_zpszdu.webp',
      productId: 24,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------12

    {
      id: 45,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/7hatbit__0_gdwhnu.webp',
      productId: 25,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 46,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/7hatbit__1_gtdoud.webp',
      productId: 25,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------13

    {
      id: 47,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/batdaucauhoitaisao__0_jjfzfn.webp',
      productId: 26,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 48,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760829/ProductImage/batdaucauhoitaisao__1_g1ef5e.webp',
      productId: 26,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------14

    {
      id: 49,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/luathapdan_o2zgkp.webp',
      productId: 27,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 50,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760830/ProductImage/luathapdan__1_njale3.webp',
      productId: 27,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------15

    {
      id: 51,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760827/ProductImage/3nguoithay_mltn2b.webp',
      productId: 28,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 52,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760828/ProductImage/3nguoithay__1_y3xhim.webp',
      productId: 28,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------16

    {
      id: 53,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/sucmanhteimthuc__0_tgwckq.webp',
      productId: 29,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 54,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686760831/ProductImage/sucmanhteimthuc__1_wpx4oe.webp',
      productId: 29,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------17

    {
      id: 55,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686761461/ProductImage/ikigai__0_hbh6dx.webp',
      productId: 30,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------18

    {
      id: 56,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844327/tutottoividai__0_tvqg5k.webp',
      productId: 31,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 57,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844327/tutottoividai__1_yqlnjk.webp',
      productId: 31,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------19

    {
      id: 58,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844326/chientrangtinte__0_cmljbg.webp',
      productId: 32,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 59,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844326/chientrangtinte__1_q9il9b.webp',
      productId: 32,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // // ----------------------20

    {
      id: 60,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844327/kynangbanhang__0_pgrtz0.webp',
      productId: 33,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 61,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686844326/kynangbanhang__1_t94m7f.webp',
      productId: 33,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },

    // ---------------------------
    {
      id: 3,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754418/ProductImage/ip14__4_ye5let.jpg',
      productId: 2,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 4,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754418/ProductImage/ip14__1_wx5myz.jpg',
      productId: 2,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 5,
      value:
        'hhttps://res.cloudinary.com/de5pwc5fq/image/upload/v1686754419/ProductImage/ip14__2_eookvy.jpg',
      productId: 2,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 6,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754418/ProductImage/ip14__3_rht1o2.jpg',
      productId: 2,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 7,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754417/ProductImage/ato__0_jkd9ng.jpg',
      productId: 3,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 8,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1686754417/ProductImage/ato__1_hcrzs5.jpg',
      productId: 3,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 9,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221247/ProductImage/tritue__0_bphssy.jpg',
      productId: 4,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 10,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221247/ProductImage/pay__0_rfcagc.jpg',
      productId: 5,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 11,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221247/ProductImage/pay__1_i11ehv.jpg',
      productId: 5,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 12,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221246/ProductImage/ip12__0_yiiy01.jpg',
      productId: 6,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 13,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221246/ProductImage/macpro__0_gbrhub.webp',
      productId: 7,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 14,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221246/ProductImage/macpro__1_cpdtth.webp',
      productId: 7,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 15,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221247/ProductImage/macpro__2_xmmpcn.webp',
      productId: 7,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 16,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221246/ProductImage/lg__0_nhj5y2.webp',
      productId: 8,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 17,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221244/ProductImage/adidas__0_ljb4fy.jpg',
      productId: 9,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 18,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221244/ProductImage/adidas__1_bjdcan.jpg',
      productId: 9,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 19,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221247/ProductImage/sh__0_nyzwmf.webp',
      productId: 10,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 20,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221248/ProductImage/tui__0_fwq3d7.webp',
      productId: 11,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 21,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221441/ProductImage/dongho__0_lhrlk1.png',
      productId: 12,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      id: 22,
      value:
        'https://res.cloudinary.com/de5pwc5fq/image/upload/v1682221247/ProductImage/mayanh__0_h5bmkd.webp',
      productId: 13,
      attributeId: 26,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
  ],
};
